#!/usr/bin/env python3
"""
IRAS GST Register Batch Checker - Command Line Script
Usage: python batch_script.py input.xlsx [--output output.xlsx] [--env sandbox|production]
"""
import os
import sys
import json
...
def main_process(args):
    ...
    print(f"🚀 Starting batch lookup for {len(uens)} UENs (sequential processing)...\n")

    # Create client and process batch
    client = IRASClient(base_url, client_id, client_secret)
    results = batch_lookup(client, uens, progress_callback=print_progress)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="IRAS GST Register Batch Checker - Command Line Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python batch_script.py input.xlsx
  python batch_script.py input.xlsx --output results.xlsx
  python batch_script.py input.xlsx --env sandbox
...
Note:
  This script uses synchronous requests (sequential processing).
  For faster interactive use with UI, consider using the Streamlit versions.
        """,
    )
